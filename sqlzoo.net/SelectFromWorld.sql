...
/*Show the name for the countries that have a population of at least
...
Show the countries that are big by area or big by population. Show name, population
and area.
*/
...
of the continent 'South America'. Use the ROUND function to show the values to two
decimal places.
...
select name, round(population/1000000,2), round(gdp/1000000000,2) from world where
...
FROM world
WHERE name LIKE '%a%'
  AND name LIKE '%e%' AND name LIKE '%i%' AND name LIKE '%o%' AND name LIKE '%u%' and name
  not like '% %'